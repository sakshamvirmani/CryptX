...
const TrendingCoins = () => {
  ...
  return (
    <div className={"relative top-6 sm:mr-4"}> {   /* agar mobile pe center nahi hora toh remove this margin right  */}
      <p className="text-4xl font-bold justify-center items-center flex  relative mt-28 mb-11 ">Market Trend </p>

      <div className={styles.trendingHold}>

        {topCoins.length > 0 ?
          <AliceCarousel autoPlay={true} autoPlayInterval={2000} animationDuration={2000} mouseTracking infinite={true} disableButtonsControls={true} disableDotsControls={true} responsive={responsive}>
            {topCoins.map((item, index) => {
              return (
                <div key={item.id + index} className="boxshDark bg-yellow-100 w-80 h-44 rounded-md flex flex-col">
